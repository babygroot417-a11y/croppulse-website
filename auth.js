...
if(!valid){show('Firebase is not configured.');}
 ...
 document.getElementById('sendOtp').addEventListener('click',async()=>{
   ...
   try{show('Sending verification codeâ€¦',false); const btn=document.getElementById('sendOtp');btn.disabled=true; confirmationResult=await signInWithPhoneNumber(auth,phone,verifier); phoneBox.hidden=true;otpBox.hidden=false;show('OTP sent. Check your phone.',false);}
   catch(e){show(e.code==='auth/operation-not-allowed'?'Phone sign-in is not enabled in Firebase Console.':(e.message||'Unable to send OTP.'));document.getElementById('sendOtp').disabled=false;try{verifier.clear();verifier=new RecaptchaVerifier(auth,'recaptcha-container',{size:'invisible'});}catch(_){} }
 });
 document.getElementById('verifyOtp').addEventListener('click',async()=>{if(!confirmationResult){show('Request an OTP first.');return;}const code=otpInput.value.trim();if(!/^\d{6}$/.test(code)){show('Enter the 6-digit OTP.');return;}try{show('Verifyingâ€¦',false);await confirmationResult.confirm(code);goDashboard();}catch(e){show('Invalid or expired OTP. Please try again.');}});
 document.getElementById('changeNumber').addEventListener('click',e=>{e.preventDefault();phoneBox.hidden=false;otpBox.hidden=true;document.getElementById('sendOtp').disabled=false;show('');});
 document.getElementById('googleBtn').addEventListener('click',async()=>{try{show('Opening Google sign-inâ€¦',false);await signInWithPopup(auth,provider);goDashboard();}catch(e){show(e.code==='auth/popup-blocked'?'Allow popups for Google sign-in.':(e.message||'Google sign-in failed.'));}});
 document.getElementById('emailSignIn').addEventListener('click',async()=>{const email=document.getElementById('email').value.trim();const password=document.getElementById('password').value;if(!email||!password){show('Enter your email and password.');return;}try{show('Signing inâ€¦',false);await signInWithEmailAndPassword(auth,email,password);goDashboard();}catch(e){show('Email or password is incorrect, or email sign-in is not enabled.');}});
 document.getElementById('forgotPassword').addEventListener('click',async e=>{e.preventDefault();const email=document.getElementById('email').value.trim();if(!email){show('Enter your email first, then click Forgot password.');return;}try{await sendPasswordResetEmail(auth,email);show('Password reset link sent. Check your email.',false);}catch(e){show(e.code==='auth/operation-not-allowed'?'Email/password sign-in is not enabled in Firebase Console.':(e.message||'Unable to send reset email.'));}});
}
