...
if (!valid) {
  ...
  const fullPhone = () => {
    ...
    return country.value + raw.replace(/\D/g, '');
  };

  document.getElementById('sendOtp').addEventListener('click', async () => {
    const phone = fullPhone();
    if (!/^\+[1-9]\d{7,14}$/.test(phone)) {
      show('Enter a valid phone number with country code.');
      return;
    }
    ...
    try {
      btn.disabled = true;
      show('Sending verification code…', false);
      confirmationResult = await signInWithPhoneNumber(auth, phone, verifier);
      phoneBox.hidden = true;
      otpBox.hidden = false;
      otpBox.classList.add('show');
      otpInput.focus();
      show('OTP sent. Check your phone.', false);
    } catch (e) {
  ...
  document.getElementById('verifyOtp').addEventListener('click', async () => {
    if (!confirmationResult) {
      show('Request an OTP first.');
      return;
    }
    const code = otpInput.value.trim();
    if (!/^\d{6}$/.test(code)) {
      show('Enter the 6-digit OTP.');
      return;
    }
  ...
  async function resolveEmail(identifier) {
    ...
    const usernameSnap = await getDoc(doc(db, 'usernames', value.toLowerCase()));
    if (!usernameSnap.exists()) return null;
    return usernameSnap.data().email || null;
  }
  ...
  document.getElementById('emailSignIn').addEventListener('click', async () => {
    ...
    if (!identifier || !password) {
      show('Enter your email/username and password.');
      return;
    }
    ...
    try {
      ...
      if (!email) {
        show('No account was found for that email or username.');
        return;
      }
  ...
  function openReset() {
    ...
    resetEmail.value = emailInput.value.includes('@') ? emailInput.value.trim() : '';
  ...
  document.getElementById('sendReset').addEventListener('click', async () => {
    ...
    try {
      ...
      resetMessage.textContent =
        e.code === 'auth/operation-not-allowed'
          ? 'Email/password authentication is disabled. Enable Email/Password in Firebase Authentication.'
          : e.code === 'auth/invalid-email'
            ? 'That email address is invalid.'
            : e.code === 'auth/unauthorized-continue-uri'
              ? 'Add thecroppulse.in to Firebase Authentication → Authorized domains.'
